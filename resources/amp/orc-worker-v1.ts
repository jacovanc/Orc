	...
	amp.registerTool({
		...
		inputSchema: {
			...
			properties: {
				...
				outcome: { type: 'string', enum: ['success', 'blocked', 'proof_complete', 'pass', 'fail'] },
		...
		execute: async (input, ctx) => {
			...
			await verifyReportEvidence(context, reportKind, reportUrl, reportCommentId)
			...
			if (context.agent_mode === 'real_development' && outcome === 'success') {
				...
				await verifyPublication(context, branch, pullRequestNumber, pullRequestUrl)
...
async function verifyReportEvidence(
	...
	const targetNumber = context.agent_mode === 'real_qa'
	...
	const expectedWebPath = `/${context.github_repository}/${context.agent_mode === 'real_qa' ? 'pull' : 'issues'}/${targetNumber}`.toLowerCase()
	if (
...
async function capabilityCall(
	...
	threadId: string,
	action: 'context' | 'report' | 'publication' | 'complete',
