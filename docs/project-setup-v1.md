...
4. The prompt assumes no existing Orc installation. The agent loads Amp's built-in `building-plugins` guidance, downloads the immutable `orc-worker-v4.ts` artifact from the deployed Orc domain, verifies the prompt-pinned SHA-256, inspects it, and publishes only that file to the authenticated user's writable **Personal Plugins** repository. It preserves unrelated plugins. A project, workspace, or machine-only installation is insufficient because later workflow stages run in fresh Orbs.
...
The setup is therefore two-phase: Personal Worker bootstrap, then project pairing. It never assumes `orc_setup_project` already exists. Worker v4 contains only `orc_setup_project`, `workflow_verify_project_connection`, and `workflow_complete`; it adds explicit direct-tool completion guidance while ordinary GitHub work continues to use Amp's native tools. Immutable Workers v2 and v3 remain available for historical setup prompts and connections.
