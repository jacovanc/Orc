...
5. **Human Review Explanation:** implemented as immutable workflow v5 with a fresh read-only agent that answers GitHub-hosted questions and returns to Human Review.
6. **Versioned stage instructions:** implemented for owner-scoped Development, QA, Explanation, and Merge task bodies, with immutable run-start snapshots. Model selection, fixed-envelope editing, and broader stage/tool configuration remain deferred.
7. **Remaining hardening:** deeper observability, backup/restore exercises, and security review.
8. **Configurable workflow editor:** new immutable graph versions with validation and preview.
9. **Incremental onboarding:** setup protocol v1 now provides a short-lived agent-assisted connection prompt; broader account/project discovery remains deferred.
