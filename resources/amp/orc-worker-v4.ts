...
export default function (amp: PluginAPI) {
	...
	amp.registerTool({
		...
		description: 'Call this registered tool directly—not through tool_search, code_exec, shell, plugin reload, or imported plugin source. It verifies GitHub evidence created with native tools, binds it to this exact attempt, and completes the Orc stage.',
		inputSchema: {
			...capabilitySchema(),
			properties: {
				...capabilitySchema().properties,
				outcome: { type: 'string', enum: ['success', 'blocked', 'proof_complete', 'pass', 'fail', 'completed', 'merged', 'requires_review'] },
...
function validRuntimeConfig(config: Record<string, unknown> | undefined, ampProjectId: string) {
	return config !== undefined
		...
		&& config.controllerProtocolVersion === 3
...
async function verifyReportEvidence(
	...
	const pullRequestReport = ['real_qa', 'real_explanation', 'real_merge'].includes(context.agent_mode)
