...
## Human-review Explanation loop

Workflow v5 adds the optional `Human Review → Ask questions → Explanation → Human Review` loop. The human writes questions directly on the exact GitHub pull request and then selects Ask questions in Orc. A fresh `real_explanation` GPT-5.6 Sol agent rereads the issue, exact PR/code, reviews, inline comments, recent human discussion, prior reports, and checks using normal native tools. The Human Review entry timestamp is only a search hint, never an authority or hard cutoff.

Explanation is read-only: it may inspect and run non-mutating checks but must not edit implementation, commit, push, merge, approve, request changes, or start Development. Each unanswered inline review comment receives its answer as a direct reply in that existing GitHub review thread, preserving the file/line context and a natural place for follow-up. A top-level question without an inline reply target receives its own focused response linked to the source. The agent then publishes only a brief marker-bound completion report linking the individual answers rather than duplicating them. `completed` and clarification-needed `blocked` both return to Human Review; crashes remain failures. Orc stores only the verified completion-report identifier/URL and never stores question or answer text.

## Versioned task bodies

Project settings exposes the exact substantive plain-text task body for Development, Independent QA, Explanation, and Merge. These owner-scoped bodies are separate from the fixed trusted launch envelope that enforces exact repository/issue/PR, native authentication, evidence, role permissions, and stage capability rules. Editing a body creates an append-only Project version; it does not edit an immutable workflow definition.

`WorkflowEngine::start` snapshots all bodies needed by the selected definition. Every launch or retry for that run uses the exact snapshot and version, even if Project settings change later. The controller inserts that body exactly once into the fixed envelope; agent registration contains role/security boundaries only and does not carry a conflicting duplicate task. The UI shows run snapshots where they exist and does not fabricate exact historical bodies for legacy runs.
...
5. Verify production migration status, queue process, `/register` 404, immutable account permission, Project connection status, and authenticated workflow-v5/task-instruction UI.
