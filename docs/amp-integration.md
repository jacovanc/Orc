...
Explanation is read-only: it may inspect and run non-mutating checks but must not edit implementation, commit, push, merge, approve, request changes, or start Development. It answers with a substantive marker-bound comment on the exact PR. `completed` and clarification-needed `blocked` both return to Human Review; crashes remain failures. Orc stores only the verified report identifier/URL and never stores the question or answer text.
